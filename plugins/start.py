...
async def get_users(client: Bot, message: Message):
    ...
    await msg.edit(f"{len(users)} ᴜsᴇʀs ᴀʀᴇ ᴜsɪɴɢ ᴍᴇ")
...
async def send_text(client: Bot, message: Message):
    if message.reply_to_message:
        ...
        pls_wait = await message.reply("<i>ʙʀᴏᴀᴅᴄᴀsᴛɪɴɢ... ɪᴛ ᴡɪʟʟ ᴛᴀᴋᴇ sᴏᴍᴇ ᴛɪᴍᴇs</i>")
        for row in query:
            chat_id = int(row[0])
            try:
...
ʙʟᴏᴄᴋᴇᴅ ᴜsᴇʀ: <code>{blocked}</code>
ᴅᴇʟᴇᴛᴇᴅ ᴀᴄᴄᴏᴜᴛs: <code>{deleted}</code>
